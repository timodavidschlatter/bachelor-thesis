SELECT 
	'teilweise Bodenbedeckung' as Wert
 	, bf.art, bf.art_code
FROM
	av_ls.v_grundstueck g1
INNER JOIN av_bb.v_boflaeche bf ON ST_INTERSECTS(g1.geom, bf.geom)
WHERE 
	...
	AND ST_INTERSECTS(ST_BUFFER(g1.geom, -1), bf.geom)
	AND NOT ST_WITHIN(ST_POINTFROMTEXT('POINT (xxx yyy)',2056), bf.geom)
;